...
async def main():
    ...
    hdc_path = device_cfg.get("hdc_path", "hdc")
    serial = device_cfg.get("serial")

    # 截图和输入共用同一个 HDC 连接（serial 相同）
    capture = HdcCapture(serial=serial, hdc_path=hdc_path)
    await capture.connect()
    capture_w, capture_h = capture.native_resolution

    input_device = HdcInput(serial=serial, hdc_path=hdc_path)
