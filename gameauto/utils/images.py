"""Image helpers: dimension parsing, resizing, grid overlay.

Adapted from mobilerun/tools/helpers/images.py.
"""
...
def fit_dimensions_to_max_side(
    ...
    """Return dimensions scaled so the longest side <= max_side."""
...
def resize_image_to_max_side(
    ...
    """Resize image bytes to fit within max_side."""
...
def resize_image_to_max_side_with_grid(
    ...
    When use_normalized=True, grid labels show [0-1000] values.
    """
    width, height = image_dimensions(image)
    target_width, target_height = fit_dimensions_to_max_side(width, height, max_side)
    ...
    with Image.open(BytesIO(image)) as source:
        ...
        _draw_coordinate_grid(screenshot, divisions=divisions, use_normalized=use_normalized)
...
def _draw_coordinate_grid(
    ...
    for index in range(divisions + 1):
        ...
        color = major_line_color if index in {0, divisions // 2, divisions} else line_color
        draw.line([(x, 0), (x, height - 1)], fill=color, width=1)
        draw.line([(0, y), (width - 1, y)], fill=color, width=1)

        x_label = f"x={round(index * max_norm / divisions)}" if use_normalized else f"x={x}"
        y_label = f"y={round(index * max_norm / divisions)}" if use_normalized else f"y={y}"

        _draw_grid_label(draw, x_label, (min(x + 3, width - 38), 4), font, label_fill, label_shadow, label_bg)
        _draw_grid_label(draw, y_label, (4, min(y + 3, height - 14)), font, label_fill, label_shadow, label_bg)

    image.alpha_composite(overlay)


def _draw_grid_label(draw, text, position, font, fill, shadow, background):
    x, y = position
    bbox = draw.textbbox((x, y), text, font=font)
    draw.rectangle((bbox[0] - 2, bbox[1] - 1, bbox[2] + 2, bbox[3] + 1), fill=background)
