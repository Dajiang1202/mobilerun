...
def _call_vlm_sync(client: OpenAI, model: str, messages: list, timeout: int = 120) -> str:
    """同步调用 VLM，不阻塞事件循环（通过 asyncio.to_thread 调用）"""
...
async def main():
    ...
    for rnd in range(1, ROUNDS + 1):
        ...
        print("Calling VLM (sync)...")
        t0 = time.time()
        content = await asyncio.to_thread(_call_vlm_sync, client, model, messages)
        ...
        if not step_results:
            print("No match: no valid swap found")
            continue
        ...
        for step_idx, step_result in enumerate(step_results):
            ...
            print(f"   Match: {step_result['match_description']}")
            ...
            try:
                ...
                print(f"   Swipe {step_idx + 1} executed")
                success_count += 1
            except Exception as e:
                print(f"   Swipe {step_idx + 1} failed: {e}")
                continue
    ...
    print(f"\n{'=' * 50}")
    print(f"Done: {success_count}/{max_total} swipes, total {elapsed:.1f}s")
    print(f"{'=' * 50}")
